...
.apihead{width: 900px;height: 10%;background: #e9e3d5;padding: 10px;font-weight: bold;vertical-align: top;color: #427f2f;text-align:right;}
.apidata{text-align:right;width: 100%;padding: 10px;font-weight: bold;vertical-align: top;background: #fff;width: 350px;padding: 10px;vertical-align: top;}
...
function selectNewProduct(){
	$.get('./index/selectNewProduct', {}, function(data) {
		drawDiv(data);
	}, 'xml');
}
function drawDiv(data) {	
	...
	const $productArr = $root.children();
	const $div = $("<div class='indexRDproducts'></div>")
	$productArr.each((i,product)=>{
		console.log(i,product);
		const $no = $(product).children("pNo");
		const $title = $(product).children("pTitle");
		const $price = $(product).children("pPrice");
		const $userId = $(product).children("pUserId");
		const $regDate = $(product).children("pRegDate");
		const $img = $(product).children("img");
		$div.append(`<div class="indexRDdata"><img class="indexRDimg" src="<%= request.getContextPath() %>/upload/product/\${$img.text()}"/><a href="\${$no.text()}">\${$title.text()}</a><p>\${$price.text()}</p><p>\${$userId.text()}</p><p>\${$dpr7.text()}</p></div>`);
	});
$('.RDproducts').html($div);
};
